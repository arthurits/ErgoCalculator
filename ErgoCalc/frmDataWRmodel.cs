    ...
    public partial class frmDataWRmodel : Form, IChildData
    {
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < gridVariables.ColumnCount; i++)
            {
                ...
                item._dPointsX = new double[item._nPuntos];
                item._dPointsY = new double[item._nPuntos];
        ...
        private void DataToGrid(List<datosWR> data)
        {
            ...
            for (var j = 0; j < nDataNumber; j++)
            {
                ...
                gridVariables[j, 3].Value = string.Join(" ", data[j]._dWork);
                gridVariables[j, 4].Value = string.Join(" ", data[j]._dRest);
        ...
        private int ComputeNumberOfPoints(datosWR data)
        {
            ...
            double nSize = data._bCiclos * data._dWork.Length + 1;
            foreach (double d in data._dRest)
            {
