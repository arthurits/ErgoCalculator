    ...
    namespace Models
    {
        ...
        namespace NIOSHModel
        {
            ...
            public class cModelNIOSH
            {
                ...
                private static extern double CalculateNIOSH_DLL([In, Out] SubTask[] datos, int[] orden, ref int nSize);
                ...
                public cModelNIOSH(List<SubTask> data, int[] order, bool composite)
                    :this()
                {
                    _data = data;
                    _order = order;
                ...
                public double CalculateNIOSH(List<SubTask> datos, int[] orden, ref int nSize)
                {
                    var tempArray = datos.ToArray();
                    _result = CalculateNIOSH_DLL(tempArray, orden, ref nSize);
                    _data = new(tempArray);
                    _order = orden;
                ...
                public override string ToString()
                {
                    ...
                    for (i = 0; i < length; i++) ordenacion[_order[i]] = length - i;

                    for (i = 0; i < length; i++)
                    {
                        strLineD[0] += "\t\t" + "Task " + ((char)('A' + i)).ToString();
                        strLineD[1] += "\t\t" + _data[i].data.weight.ToString();
                        strLineD[2] += "\t\t" + _data[i].data.h.ToString();
                        strLineD[3] += "\t\t" + _data[i].data.v.ToString();
                        strLineD[4] += "\t\t" + _data[i].data.d.ToString();
                        strLineD[5] += "\t\t" + _data[i].data.f.ToString();
                        strLineD[6] += "\t\t" + _data[i].data.fa.ToString();
                        strLineD[7] += "\t\t" + _data[i].data.fb.ToString();
                        strLineD[8] += "\t\t" + _data[i].data.td.ToString();
                        strLineD[9] += "\t\t" + _data[i].data.a.ToString();
                        strLineD[10] += "\t" + _data[i].data.c;

                        strLineR[0] += "\t\t" + "Task " + ((char)('A' + i)).ToString();
                        strLineR[1] += "\t\t" + _data[i].factors.LC.ToString("0.####");
                        strLineR[2] += "\t\t" + _data[i].factors.HM.ToString("0.####");
                        strLineR[3] += "\t\t" + _data[i].factors.VM.ToString("0.####");
                        strLineR[4] += "\t\t" + _data[i].factors.DM.ToString("0.####");
                        strLineR[5] += "\t\t" + _data[i].factors.FM.ToString("0.####");
                        strLineR[6] += "\t\t" + _data[i].factors.FMa.ToString("0.####");
                        strLineR[7] += "\t\t" + _data[i].factors.FMb.ToString("0.####");
                        strLineR[8] += "\t\t" + _data[i].factors.AM.ToString("0.####");
                        strLineR[9] += "\t\t" + _data[i].factors.CM.ToString("0.####");
                        strLineR[10] += "\t\t" + _data[i].indexIF.ToString("0.####");
                        if (_composite == true)
                            strLineR[11] += "\t";
                        strLineR[11] += "\t" + _data[i].LI.ToString("0.####");
                    ...
                    for (i = 0; i < length; i++) ordenacion[i] = _order[length - i - 1];
                    ...
                    if (length > 1)
                    {
                        if (_composite == true)
                        {
                            ...
                            strResult.Append("Tasks order:\t\t" + strLineR[12] + "\n\n");
                        }
                        else
                        {
                    ...
                    if (length > 1)
                    {
                        if (_composite == true)
                        {
                            var strName = ((char)('A' + ordenacion[0])).ToString();
                            strEquationT = "CLI = Index(" + strName + ")"; //((char)('A' + i)).ToString()
                            strEquationN = "CLI = " + _data[ordenacion[0]].LI.ToString("0.####");
                            for (i = 1; i < length; i++)
                            {
                                strName = ((char)('A' + ordenacion[i])).ToString();
                                strEquationT += " + IndexIF(" + strName + ") * (1/FMa(" + strName + ") - 1/FMb(" + strName + "))";
                                strEquationN += " + " + _data[ordenacion[i]].indexIF.ToString("0.####") + " * (1/" + _data[ordenacion[i]].factors.FMa.ToString("0.####") + " - 1/" + _data[ordenacion[i]].factors.FMb.ToString("0.####") + ")";
                            }
                            strEquationN += " = " + _result.ToString("0.####");
                            strResult.Append(strEquationT + "\n");
                            strResult.Append(strEquationN + "\n\n");
                            strResult.Append("The NIOSH lifting index is: " + _result.ToString("0.####") + "\n");
                        }
                        else
                        {
                            ...
                            for (i = 0; i < length; i++)
                            {
                                strEquationN = "LI = " + _data[i].data.weight.ToString("0.####") + " / (";
                                strEquationN += _data[i].factors.LC.ToString("0.####") + " * ";
                                strEquationN += _data[i].factors.HM.ToString("0.####") + " * ";
                                strEquationN += _data[i].factors.VM.ToString("0.####") + " * ";
                                strEquationN += _data[i].factors.DM.ToString("0.####") + " * ";
                                strEquationN += _data[i].factors.FM.ToString("0.####") + " * ";
                                strEquationN += _data[i].factors.AM.ToString("0.####") + " * ";
                                strEquationN += _data[i].factors.CM.ToString("0.####") + ") = ";
                                strEquationN += _data[i].LI.ToString("0.####");
                                strResult.Append(strEquationN + "\n");
                            }
                            strResult.Append("\n");
                        }
                    ...
                    {
                        strEquationT = "LI = Weight / (LC * HM * VM * DM * FM * AM * CM)";
                        strEquationN = "LI = " + _data[0].data.weight.ToString("0.####") + " / (";
                        strEquationN += _data[0].factors.LC.ToString("0.####") + " * ";
                        strEquationN += _data[0].factors.HM.ToString("0.####") + " * ";
                        strEquationN += _data[0].factors.VM.ToString("0.####") + " * ";
                        strEquationN += _data[0].factors.DM.ToString("0.####") + " * ";
                        strEquationN += _data[0].factors.FM.ToString("0.####") + " * ";
                        strEquationN += _data[0].factors.AM.ToString("0.####") + " * ";
                        strEquationN += _data[0].factors.CM.ToString("0.####") + ") = ";
                        strEquationN += _data[0].LI.ToString("0.####");

                        strResult.Append(strEquationT + "\n");
                        strResult.Append(strEquationN + "\n\n");
                        strResult.Append("The NIOSH lifting index is: " + _data[0].LI.ToString("0.####") + "\n");
                    };
            ...
            public class cModelStrain
            {
                ...
                */
                ...
                private ModelTask TaskfromMarshal(IntPtr ptrTask)
                {
                    ...
                    task.h = BitConverter.Int64BitsToDouble(Marshal.ReadInt64(ptrTask, memoffset));
                    memoffset += sizeDouble;
                    task.ha = BitConverter.Int64BitsToDouble(Marshal.ReadInt64(ptrTask, memoffset));
                    memoffset += sizeDouble;
                    task.hb = BitConverter.Int64BitsToDouble(Marshal.ReadInt64(ptrTask, memoffset));
                    memoffset += sizeDouble;
                    task.HM = BitConverter.Int64BitsToDouble(Marshal.ReadInt64(ptrTask, memoffset));
                    memoffset += sizeDouble;
                    task.HMa = BitConverter.Int64BitsToDouble(Marshal.ReadInt64(ptrTask, memoffset));
                    memoffset += sizeDouble;
                    task.HMb = BitConverter.Int64BitsToDouble(Marshal.ReadInt64(ptrTask, memoffset));
                    memoffset += sizeDouble;
